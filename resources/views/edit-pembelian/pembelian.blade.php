        ...
        <div class="row g-2">
            <div class="col-12 mb-2 order-0">
                <div class="card">
                    ...
                    <!-- Form Pembelian -->
                    <div class="container">
                        <form method="POST" action="{{ route('pembelian.update', $transaksi->id) }}"
                                            ...
                                            <select class="form-select select2" name="pajak_id" id="pajakSelect">
                                                ...
                                                @foreach ($pajaks as $p)
                                                    <option value="{{ $p->id }}" data-value="{{ $p->nilai }}"
                                                        {{ $transaksi->pajak_id == $p->id ? 'selected' : '' }}>
                                                        {{ $p->nama }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                ...
                                <div class="row">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <select class="form-select select2" name="pembayaran_id" id="pembayaranSelect"
                                                ...
                                                @foreach ($pembayarans as $p)
                                                    <option value="{{ $p->id }}"
                                                        {{ $transaksi->pembayaran_id == $p->id ? 'selected' : '' }}>
                                                        {{ $p->nama }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    ...
                                    <div class="col-3">
                                        <div class="mb-3 d-none" id="buktiWrapper">
                                            ...
                                            @if ($transaksi->bukti)
                                                <div id="previewBukti" class="mt-2">
                                                    <img src="{{ asset('storage/bukti/' . $transaksi->bukti) }}"
...
@section('js')
    ...
    <script>
        produkList = {!! json_encode(
            $transaksi->detailTransaksis->map(function ($item) {
                return [
                    'id' => $item->produk_id,
