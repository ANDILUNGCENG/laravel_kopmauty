        ...
        <div class="header mb-3">
            <div class="row align-items-center">
                <form class="d-flex w-100" id="searchForm">
                    ...
                    <input type="hidden" name="transaksi_id" value="{{ $transaksi->id }}" id="transaksi_id_edit_kasir">
                </form>
            </div>
        </div>
        <!-- Garis Pembatas -->
                ...
                @foreach ($produks_all as $produk)
                    <div class="product-card" data-product-barcode="{{ $produk->barcode }}">
                        <div class="card h-100 shadow-sm product-card position-relative"
                            data-product-barcode="{{ $produk->barcode }}" data-product-id="{{ $produk->id }}"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const transaksiId = urlParts[urlParts.length - 1]; // ambil bagian terakhir dari URL
        const beepSound = document.getElementById('beepSound');
        let isAddingToCart = false;

        function addProductToCart(productId, stock, transaksiId) {
            if (isAddingToCart) return;
            isAddingToCart = true;

            fetch(`/tambah-produk-transaksi/${productId}/${transaksiId}`, {
                    method: 'POST',
                    headers: {
        ...
        productCards.forEach(card => {
            card.addEventListener('click', function() {
                ...
                addProductToCart(productId, stock, transaksiId);
            });
        });
    });
</script>
...
<script>
    ...
    const searchInput = document.getElementById('searchInput');
    // const transaksi_id = document.getElementById('transaksi_id_edit_kasir');

    let transaksi_id = document.getElementById("transaksi_id_edit_kasir").value;
    ...
    searchForm.addEventListener('submit', function(e) {
        ...
        fetch(`{{ route('transaksi.barcode.update') }}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
            ...
            .catch(err => {
                console.error(err);
                showToast('Terjadi kesalahan saat menambahkan produk.');
            })
            .finally(() => {
                isAddingToCart = false;
            });
